...
This library runs on the "Raspberry Pi":http://www.pololu.com/product/2752 (Model B+, A+, or Pi 2) and provides functions for controlling both channels of the "Pololu DRV8835 Dual Motor Driver Kit for Raspberry Pi":http://www.pololu.com/product/2753.  This library supports both Python 2.7.x and Python <notextile>3.2.x</notextile>.
...
These instructions also assume you will use Python <notextile>2.x</notextile>.  If you want to use Python <notextile>3.x</notextile>, you will need to replace @python@ with @python3@ in the names of the @apt-get@ packages below, and use the @python3@ command instead of @python@ for running Python scripts, and use the @pip3@ command instead of @pip@ to install pip packages.

To download and install WiringPi-Python, run:
